...
import os
if os.environ.get("NANOCHAT_DISABLE_EXPANDABLE_SEGMENTS") != "1":
...
# In some virtualized multi-GPU environments, importing CUDA-aware modules before
# binding each rank to its GPU can leave rank-local CUDA context on the wrong
# device. Bind early, before importing the rest of nanochat.
...
# Keep this on CPU initially. It is only used by evaluation, and some
# virtualized GPU environments are unstable when moving this tensor to CUDA
# during early process startup.
...
if os.environ.get("NANOCHAT_DISABLE_COMPILE") == "1":
    print0("Skipping torch.compile because NANOCHAT_DISABLE_COMPILE=1")
else:
